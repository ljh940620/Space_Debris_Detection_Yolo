...
git clone [https://github.com/](https://github.com/)[ljh940620]/space-hazard-detection-yolo.git
cd space-hazard-detection-yolo
...
데이터셋 확장: 기존 위성/쓰레기 데이터셋에 'D2' (우주 파편) 클래스를 추가하여 총 12개 클래스로 데이터셋을 확장했습니다.

환경 구성: 두 YOLO 나노 모델의 성능을 공정하게 비교하기 위해, 동일한 하드웨어와 소프트웨어 환경을 구성했습니다.

모델별 훈련 및 평가: 이 통제된 환경에서 yolov11n과 공식 yolov8n 모델을 확장된 데이터셋과 동일한 하이퍼파라미터(epochs: 50, batch: 16 등)로 각각 훈련하고, test 데이터셋으로 최종 성능을 평가했습니다.
...
지표 (Metric)	yolov11n (자체 모델)	yolov8n (공식 모델)
...
yolov11n은 **정밀도(Precision)**가 더 높아, 예측한 결과의 신뢰도(오탐지가 적음)가 중요한 경우 더 적합합니다.

yolov8n은 **재현율(Recall)**이 약간 더 높아, 객체를 하나라도 놓치지 않는 것이 중요한 경우 더 적합할 수 있습니다.

신규 클래스('D2') 탐지 성능: 두 모델 모두 새롭게 추가된 'D2' 클래스에 대해서는 99% 이상의 mAP50 점수를 기록하며 매우 성공적으로 학습했음을 확인했습니다.
