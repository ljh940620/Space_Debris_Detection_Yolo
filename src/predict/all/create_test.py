...
[이미지 + 정보창] 타일 16개를 합친 4x4 그리드 요약 이미지를 생성합니다.
(가독성 극대화 최종 버전)
"""
# --- 1. 경로 및 설정 --- (이전과 동일)
...
# 타일 및 그리드 스타일 설정
IMG_SIZE = (350, 350)       # 몽타주에 들어갈 이미지 영역 크기
LEGEND_WIDTH = 250          # 정보창 너비
TILE_SIZE = (IMG_SIZE[0] + LEGEND_WIDTH, IMG_SIZE[1]) # 최종 타일 크기
GRID_SHAPE = (4, 4)
...
def create_tile_with_legend(image, boxes, is_prediction=False):
    ...
    for i, box_data in enumerate(boxes):
        ...
        if is_prediction:
            text_line2 = f"   (Conf: {conf:.3f})"
            (w2, h2), _ = cv2.getTextSize(text_line2, FONT, font_scale_conf, font_thickness)
            cv2.putText(tile, text_line2, (IMG_SIZE[0] + 45, y_pos), FONT, font_scale_conf, (200, 200, 200), 1)
...
def main():
    ...
    for class_id, class_name in enumerate(tqdm(CLASS_NAMES, desc="클래스별 처리 중")):
        ...
        cv2.imwrite(os.path.join(class_specific_output_dir, "Label_yolo11.jpg"), gt_montage)
        cv2.imwrite(os.path.join(class_specific_output_dir, "Prediction_yolo11.jpg"), pred_montage)
