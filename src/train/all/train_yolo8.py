...
YOLO('yolov8n.pt'): YOLO 전문가에게 'YOLOv8' 모델 중에서 가장 작고 빠른 'nano' 버전인 yolov8n.pt 선수를 선발하라고 지시합니다.
...
#model_path = os.path.join(PROJECT_ROOT, 'model', 'yolov8s.pt') # yolov8n.pt -> yolov8s.pt
...
name='hazard_yolov8_hazard_only_run': "이번 훈련의 모든 결과물(성적표, 성장일지, 최종 결과물 등)은 runs/detect/ 폴더 아래에 hazard_yolov8_hazard_only_run 이라는 이름의 폴더를 만들어 저장해라." """
# 3. 모델 학습 시작!
...
if __name__ == '__main__':
    results = model.train(
        ...
        #name = 'yolo8_hazard_run',
        name = 'yolo8_hazard_all',
    ...
    한 문장으로 요약하자면, 이 스크립트는 "똑똑한 YOLOv8n 학생을 데려와서(YOLO(...)), data.yaml이라는 시간표를 주고(data=...), 50일 동안(epochs=50) 8쪽씩(batch=8) 공부시켜서, 그 결과를 hazard_yolov8... 라는 이름의 노트에 기록하는" 과정이라고 할 수 있습니다. """
