...
결과를 시각화하고 저장하는 최종 스크립트입니다.
"""

# --- 1. 경로 및 설정 ---
CURRENT_FILE_PATH = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(CURRENT_FILE_PATH)))
# [중요] 분석할 모델의 경로를 정확하게 지정해주세요.
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'final_comparison_results')
...
def create_legend_panel(title, boxes, is_prediction=False):
    ...
    panel = np.full((IMG_SIZE[1] // 2, LEGEND_WIDTH, 3), 40, dtype=np.uint8)
...
def main(image_path):
    ...
    # 정보창 위아래로 합치기
    legend_panel_combined = np.vstack([gt_legend_panel, pred_legend_panel])
    
    # 최종 이미지 크기에 맞게 패널 크기 조정
    gt_image_panel = cv2.resize(gt_image_panel, IMG_SIZE)
    pred_image_panel = cv2.resize(pred_image_panel, IMG_SIZE)
    legend_panel = cv2.resize(legend_panel_combined, (LEGEND_WIDTH, IMG_SIZE[1]))
    ...
    # 화면에 보여줄 때는 적절한 크기로 조정 (선택 사항)
    display_h, display_w, _ = final_image.shape
    if display_w > 1920:
        scale = 1920 / display_w
        display_h = int(display_h * scale)
        final_image_display = cv2.resize(final_image, (1920, display_h))
    else:
        final_image_display = final_image

    cv2.imshow("Label vs Prediction", final_image_display)
